...
{
  if (tracing_yajl_gen) {
    yajl_free(tracing_yajl_gen);
    tracing_yajl_gen = NULL;
  }

  if (!RTEST(*argv)) {
    tracing_json_filename = Qnil;
  } else {
    tracing_yajl_gen = json_for_args(argc, argv);
...
memprof_trace_request(VALUE self, VALUE env)
{
  ...
  yajl_gen_number(gen, str_time, strlen(str_time));
  ...
  if (RTEST(ret) && TYPE(ret) == T_ARRAY) {
    yajl_gen_cstr(gen, "response");
    yajl_gen_map_open(gen);
    yajl_gen_cstr(gen, "code");
    yajl_gen_value(gen, RARRAY_PTR(ret)[0]);
    yajl_gen_map_close(gen);
  }

  yajl_gen_cstr(gen, "time");
  yajl_gen_integer(gen, end_time-start_time);

  yajl_gen_map_close(gen);
  yajl_gen_reset(gen);

  if (gen != tracing_yajl_gen)
    yajl_free(gen);
        ...
        if (th == rb_curr_thread) {
          ...
          if (th->wait_for & WAIT_TIME) {
            ...
            if (th->delay == DELAY_INFTY)
              ...
              yajl_gen_double(gen, th->delay - timeofday());
          }
